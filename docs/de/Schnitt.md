...
Beende vor dem Ausführen deine Aufnahme und denk daran, dass durch Prepare for editing alle StudioLink Verbindungen beendet werden! Wenn du später doch noch etwas aufnehmen möchtest, kannst du die Spur wieder auf Aufnahme schalten und so etwas neu aufnehmen. Danach kannst du erneut `Prepare all tracks for editing` ausführen.

Zuätzlich kannst du jetzt noch in die Schnittansicht zu wechseln. Dafür kannst du Links am Rand unter `Views` auf die Schere drücken, im Menü `Podcast -> screenset editing` aufrufen oder `F9` drücken. Diese Ansicht bietet dir eine zusätzliche Übersicht über dein Projekt.
...
### Split Items
Wie der Name schon sagt, kannst du hiermit ein Item in zwei Items aufteilen. Dafür setzt du den Edit-Curser<!-- @Todo name prüfen--> an die Stelle, an der du eine Trennung vornehmen willst und klickst dann auf den Button Nummer 5. Alernativ kannst du die Taste `S` drücken.
https://sendegate.de/t/faq-einfacher-schnitt-aller-spuren/6911

### Softer Fade-in und Fade-out
Wenn du gern am Anfang und oder Ende eine Sanfte Ein/abblenden der Lautstärken haben möchtest, kannst du mit der Maus über den Linken-oberen Rand des Items gehen. Dann siehst du schon, dass sich dein Mauszeiger in eine Kurve verändert hat. Jetzt kannst du mit der Maus klicken und halten und so eine Einblende ziehen. Diese kann beliebig lang sein. Das gleiche kannst du auch an jedem Item-Ende machen. Rechts oben mit der Maus und dann klicken und ziehen.
<!--@TODO Gif einfügen -->

### Kapitelmarken
<!-- @todo: Was passiert mit dem video?
https://www.youtube.com/watch?v=vdLpynu1ixE&t=5080s -->
In Ultraschall kannst du ganz einfach Kapitelmarken setzen, die dann später auch in den meisten Podcatchern auftauchen. In Ultraschall wird zwischen zwei Markenarten unterschieden. Kapitelmarker und Editmarker. Die Kapitelmarker können mit dem Button 3 oder der Taste `m` gesetzt werden, diese sind die Marken die später im Podcatcher auftauchen. Zusätzlich gibt es noch die Editmarker, diese können mit dem Button 4 oder `e` gesetzt werden. Diese sind nur für den Schnitt gedacht um sich stellen im Projekt zu markieren und werden nicht mit exportiert.

#### Markerdashbord
<!--@TODO: Fotos einfügen -->
Das Markerdashbord kannst du mit `alt+shift+m` öffnen. Dort kannst du deine gesetzten Kapitelmarker verwalten und exportieren.

#### Region/Markermanager
Den Region- und Markermanager findest du unten rechts. Dort kannst du zwischen der Routing Matrix, den FX Tracks und dem Region-und Markermanager wechseln. Dort werden dir alle Marker - auch die Editmarker angezeigt. Mit einem Doppeltklick auf den Namen, kannst du diese bearbeiten. In der lezten Spalte kannst du mit einem Doppelklick die Farbe des Markers auswählen.
...
Button [3] setzt eine (unbenannte) Kapitelmarke an die aktuelle Position des Play-Cursors – Button [4] setzt eine Edit-Marke. Mehr zum Thema Marken findest du im Abschnitt [Kapitelmarken](#Kapitelmarken).

#### [5]: Split
![Buttons](https://raw.githubusercontent.com/Ultraschall/ultraschall-manual/main/assets/images/Schnitt/edit-buttons-split.png)

Button [5] teilt ein Item an der aktuellen Cursor-Position. Mehr dazu findest du im Abschnitt [Split Items](#Split-Items).
...
Der Umschalter [6] aktiviert beziehungsweise deaktiviert den Maus-Selektionsmodus und beeinfluss so das Mausverhalten:
...
Button [7] entspricht der Tastenkombination `CMD`/`CTRL` + `X` – damit schneidest du den selektierten Bereich über alle Spuren hinweg aus deiner Aufnahme heraus. Alle nachfolgenden Items verschieben sich nach links, um die entstandene Lücke zu schließen.
...
Zeigt beziehungsweise versteckt die Mute-Envelopes aller Spuren. Im Abschnitt [Mute](#Mute) findest du mehr dazu.
...
Um weitere Sounds oder Aufnahmen in dein Projekt einzufügen, kannst du einfach die gewünschten Dateien per Drag & Drop in dein Projekt ziehen. Wenn du die Datei an eine Stelle im Projekt ziehst, an der keine Spur ist, erstellt Reaper eine neue Spur mit dem Inhalt der Datei und benennt diese dann nach dem Dateinamen.


![Achtung](https://raw.githubusercontent.com/Ultraschall/ultraschall-manual/main/assets/images/Allgemein/Achtung.png =20x20) **Achtung**:  Unter dem Menüpunkt `Edit -> Project Settings` kannst du einstellen wie Reaper mit diesen Mediendatein umgehen soll. Hier empfehlen wir auf jeden Fall die Einstllung `Copy media to project`. Am besten in den globalen Reaper Einstellungen unter `Options -> preferences` unter dem Punkt `Media`  das Kreuz bei `Copy imported media to project media directory` setzten. Sonst kann es passieren, dass du ungewollt Abhängigkeiten erschaffst, die dir später Probleme bereiten können.
...
Um kopierte oder ausgeschnittene Inhalte wieder einzufügen, wählst du mit einem Klick in den linken Spurbereich die Spur, auf der die Inhalte landen sollen. Anschließend setzt du den Cursor an die gewünschte Stelle und wählst `Paste` entweder über das `Edit` oder das Kontext-Menü oder vie Tastenkombination `CTRL+V` beziehungsweise `CMD+V`.
